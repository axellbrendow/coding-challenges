...
to square number I + K, providing that square number I + K exists. If square number I + K does not exist, we
throw the die again until we obtain a valid move. Finally, we mark square number I + K.
...
public class NumberSolitaire {
  public static int solution(int[] A) {
    ...
    for (int i = A.length - 3; i >= 0; i--) {
      for (int j = 1; j <= 6 && i + j < A.length; j++) {
  ...
  public static void main(String[] args) {
    assert solution(new int[]{1, -2, 0, 9, -1, -2}) == 8;
  }
}
